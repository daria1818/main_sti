...
if(!empty($_POST['email']) && !empty($_POST['count'])){
    ...
    if(count($userInfo) > 1){
        ...
        if(in_array(33, $arUserGroups)){
            if($userInfo[0]['UF_LOYALTY_COIN'] == 0){
                echo "<span style='color:red;'>У пользователя нет начисленных STICoin</span>";
            }else if($userInfo[0]['UF_LOYALTY_COIN'] < $_POST['count']){
                echo "<span style='color:red;'>У пользователя коинов меньше, чем введенное кол-во для списания. В наличии : ".$userInfo[0]['UF_LOYALTY_COIN'] . ' STICoin</span>';
            }else{
            ...
            echo "<span style='color:red;'>Добавьте пользователя в группу STICoin перед списанием</span>";
        }
    }
}
else{
