...
$contactFields = [
    'NAME' => $name,
    'LAST_NAME' => $surname,
    'COMPANY_ID' => $companyId, // Привязка контакта к компании сразу при создании
    'ASSIGNED_BY_ID' => $userBXId,
    'CREATED_BY_ID' => $userBXId,
    'DATE_CREATE' => $formattedDate,
];
...
if (!$dealResult->isSuccess()) {
    logError('Error creating deal: ' . implode(', ', $dealResult->getErrorMessages()));
    returnJson(['success' => false, 'message' => 'Ошибка создания сделки: ' . implode(', ', $dealResult->getErrorMessages())]);
}

$dealId = $dealResult->getId();

returnJson(['success' => true, 'message' => 'Курс успешно обновлен, билет забронирован, компания, контакт и сделка созданы в CRM.', 'companyId' => $companyId, 'contactId' => $contactId, 'dealId' => $dealId]);
