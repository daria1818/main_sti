...
$arParams['MESS_PRICE_BY_TARIFF'] = Loc::getMessage('PRICE_BY_TARIFF');
...
$APPLICATION->SetAdditionalCSS($templateFolder.'/scripts/select2.min.css', true);
$this->addExternalJs($templateFolder.'/scripts/select2.min.js');
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="row orderform--v1 bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<div class="bx-soa-section bx-active">
					<!--	REGION BLOCK	-->
	...
	<script>
		...
		BX.Sale.OrderAjaxComponent.init({
            ...
            prekursor: <?=$arResult['WITH_PREKURSOR'] ? '1' : '0'?>
		});
	</script>
	<script>
		<?
