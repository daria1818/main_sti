...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	...
	<?if(($arParams["AJAX_REQUEST"]=="N") || !isset($arParams["AJAX_REQUEST"])){?>
		<?if(isset($arParams["TITLE"]) && $arParams["TITLE"]):?>
			<hr/>
			<div class="top_block">
				<h3 class="title_block"><?=$arParams['TITLE'];?></h3>
				<a href="/sale_products/">Все товары</a>
			</div>
			<h5></h5>
		<?endif;?>
					...
					<div class="catalog_item main_item_wrapper item_wrap <?=(($_GET['q'])) ? 's' : ''?>" id="<?=$arItemIDs["strMainID"];?>">
						<div>
							<div class="image_wrapper_block">
								...
								<?else:?>
									<?
										if($arParams["updPict"] && $arItem["OFFERS_LINK_PICT_TO_MAIN"]){?>
											<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb shine">
												<img loading="lazy" class="img-responsive " src="<?=$arItem["OFFERS_LINK_PICT_TO_MAIN"]?>" alt="<?=$elementName?>" title="<?=$elementName?>">
											</a>
										<?}else{
							...
							<div class="footer_button <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?> inner_content js_offers__<?=$arItem['ID'];?>">
									...
									<?if($arItem["OFFERS"]){?>
										<?if(!empty($arItem['OFFERS_PROP'])){?>
											<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>" data-site_id="<?=SITE_ID;?>" data-id="<?=$arItem["ID"];?>" data-offer_id="<?=$arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["ID"];?>" data-propertyid="<?=$arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["PROPERTIES"]["CML2_LINK"]["ID"];?>" data-offer_iblockid="<?=$arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["IBLOCK_ID"];?>">
												<?$arSkuTemplate = array();?>
												<?$arSkuTemplate=CNextCustom::GetSKUPropsArray($arItem['OFFERS_PROPS_JS'], $arResult["SKU_IBLOCK_ID"], $arParams["DISPLAY_TYPE"], $arParams["OFFER_HIDE_NAME_PROPS"], "N", $arItem, $arParams['OFFER_SHOW_PREVIEW_PICTURE_PROPS']);?>
												<?foreach ($arSkuTemplate as $code => $strTemplate){
													if (!isset($arItem['OFFERS_PROP'][$code]))
														continue;
													if($arItem["~IBLOCK_SECTION_ID"] == "8374") {
														$strTemplate = str_replace('<span>Срок годности</span>', '<span>Дата курса</span>', $strTemplate);
													}else{
								...
								<input type="hidden" name="LIMIT_BASKET" value="<?=(isset($arItem['LIMIT_BASKET']) ? 'Y' : '')?>">
								<input type="hidden" name="LIMIT_SECTION_ID" value="<?=(isset($arItem['LIMIT_SECTION_ID']) ? $arItem['LIMIT_SECTION_ID'] : '')?>">
								<input type="hidden" name="SECTION_NAME" value="<?=(isset($arItem['SECTION_NAME']) ? $arItem['SECTION_NAME'] : '')?>">
								<?if(!$arItem["OFFERS"] /*|| $arParams['TYPE_SKU'] !== 'TYPE_1'*/):?>
								...
								<?elseif($arItem["OFFERS"]):?>
									<?if(empty($arItem['OFFERS_PROP'])){?>
										<div class="offer_buy_block buys_wrapp woffers">
											...
											$arAddToBasketData["HTML"] = str_replace("Подробнее", "Купить", $arAddToBasketData["HTML"]);?>
											<!--noindex-->
												<?=$arAddToBasketData["HTML"]?>
											<!--/noindex-->
										</div>
									...
									<?}?>
                                                ...
                                                <div id="<?=$arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="ce_cmp_visible mob-vers <?=(($arAddToBasketData["ACTION"] == "ORDER"/*&& !$arItem["CAN_BUY"]*/)  || !$arAddToBasketData["CAN_BUY"] || !$arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_LIST"] || $arAddToBasketData["ACTION"] == "SUBSCRIBE" ? "wide" : "");?>">
                                                    <!--noindex-->
                                                        <?=$arAddToBasketData["HTML"]?>
                                                    <!--/noindex-->
                                                </div>
