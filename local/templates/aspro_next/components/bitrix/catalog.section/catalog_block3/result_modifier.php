...
{
	$elements = CNextCache::CIBlockElement_GetList([], ['ID' => array_column($arResult['ITEMS'], 'ID')], false, false, ['ID', 'SECTION_ID', 'IBLOCK_SECTION_ID']);
	$arSections = [];
	foreach($elements as $element)
	{
		$arSections[$element['ID']] = $element['IBLOCK_SECTION_ID_SELECTED'];
	}
	$sections = CNextCache::CIBlockSection_GetList([], ['IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ID' => array_values($arSections), '!UF_LIMIT_BASKET' => false], false, ['ID', 'NAME', 'UF_LIMIT_BASKET'], false);
	if(!empty($sections))
	{
		$arSectionIds = array_column($sections, 'NAME', 'ID');
		foreach($arResult['ITEMS'] as &$item)
		{
			if(isset($arSections[$item['ID']]))
				$item['LIMIT_SECTION_ID'] = $arSections[$item['ID']];
			if(isset($arSectionIds[$arSections[$item['ID']]])){
				$item['LIMIT_BASKET'] = 'Y';
				$item['SECTION_NAME'] = $arSectionIds[$arSections[$item['ID']]];
			}
		}
	}
}
...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if(empty($arItem["DETAIL_PICTURE"]) && $arParams["updPict"]){
			if (CModule::IncludeModule("iblock")) {
			    $elementId = $arItem["PROPERTIES"]["CML2_LINK"]["~VALUE"]; // ID вашего элемента
			    $res = CIBlockElement::GetByID($elementId);
			    
			    if ($arRes = $res->GetNext()) {
			        $detailPictureId = $arRes['DETAIL_PICTURE'];
			        if ($detailPictureId) {
			            $detailPictureSrc = CFile::GetPath($detailPictureId);
			            $arNewItemsList[$key]["OFFERS_LINK_PICT_TO_MAIN"] = $detailPictureSrc;
			        }
				}
			}
		}
...
if($saleTypeID == "main"){
	foreach ($arResult["ITEMS"] as $key => $item) {
		...
		$arFilter = Array("IBLOCK_ID"=>$item['IBLOCK_ID'], "=CODE"=>$item["CODE"], "ACTIVE"=>"Y");
		$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
		while($ob = $res->Fetch()){
			$arCur = $ob;
		}
		if($arCur){
			$arResult["ITEMS"][$key]["DETAIL_PAGE_URL"] = '/products/'.$arCur['CODE'].'/';
		}else{
			$arSelect = Array("ID",'CODE');
			$arFilter = Array("IBLOCK_ID"=>$item['IBLOCK_ID'], "PROPERTY_OLDCODE"=>$item["CODE"], "ACTIVE"=>"Y");
			$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
			while($ob = $res->Fetch()){
				$arCur = $ob;
			}
	...
	foreach ($arResult["ITEMS"] as $key => $item) {
	    $ELEMENT_ID = $item["PROPERTIES"]["CML2_LINK"]["~VALUE"];
	    $res = CIBlockElement::GetByID($ELEMENT_ID);
	    
	    if ($ar_res = $res->GetNext()) {
	        $arIBlock = CIBlock::GetArrayByID($ar_res['IBLOCK_ID']);
	        $urlTemplate = $arIBlock['DETAIL_PAGE_URL'];
	        $detailPageUrl = CIBlock::ReplaceDetailUrl($urlTemplate, $ar_res, true, "E");
	        $arResult["ITEMS"][$key]["DETAIL_PAGE_URL"] = $detailPageUrl . "?oid=". $arItem["ID"];
	    }
	}
}
