...
if($arResult){
	...
	for($index = 0; $index < $cnt; ++$index){
		...
		$pageLink =  str_replace("products", "catalog", $arItem["LINK"] );
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			...
			if($arSubSections){
				if($index == ($cnt-1) && GetPagePath() === $arItem["LINK"]):
					...
					$strReturn .= '<a class="number" href="'.$pageLink.'" itemprop="item">';
				endif;
				if ($bShowMobileArrow) {
				...
				$strReturn .= '<a href="'.$pageLink.'" title="'.$title.'" itemprop="item">';
				if ($bShowMobileArrow) {
					$strReturn .= CNext::showIconSvg('colored_theme_hover_bg-el-svg', SITE_TEMPLATE_PATH.'/images/svg/catalog/arrow_breadcrumbs.svg');
				}
			...
			$strReturn .= '<span class="'.($bMobileBreadcrumbs ? ' bx-breadcrumb-item--mobile' : '').'" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><link href="'.GetPagePath().'" itemprop="item" /><span><span itemprop="name">'.urldecode($title).'</span><meta itemprop="position" content="'.($index + 1).'"></span></span>';
		}
	}
