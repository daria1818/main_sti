...
if (is_array($arElements) && !empty($arElements))
{
	...
	if($arItems){
		...
		ob_start();
		include_once 'sections_brands.php';
		$htmlSections_brands = ob_get_clean();
		$APPLICATION->AddViewContent('filter_section_brands', $htmlSections_brands);
		...
		// ob_start();
		// include_once 'filter_brands.php';
		// $htmlFilterBrands = ob_get_clean();
		// $APPLICATION->AddViewContent('filter_content_brands', $htmlFilterBrands);
			...
			<div class="catalog <?=$display;?> search js_wrapper_items" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arTransferParams, false))?>'>
				...
				<?endif;?>
					...
					<?
						$GLOBALS["arrFilter"];
						if(isset($_GET['brand'])){
							$GLOBALS["arrFilter"] = Array('PROPERTY_1501_VALUE' => $_GET['brand']);
						}
						?> 
...
<?if($bShowFilter):?>
	...
	<div class="left_block filter_visible">
		...
		// global $USER;
		// if ($USER->IsAdmin()) {
			//$APPLICATION->ShowViewContent('filter_content_brands');
			$APPLICATION->ShowViewContent('filter_section_brands');
		// }
		?>
