...
/*brand item*/
$arBrand = array();
if(!empty($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
...
if(!empty($arResult['PROPERTIES']['BONUS_PRODUCT']['VALUE'])){

	$arSelect = Array("ID", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM","DETAIL_PICTURE","PREVIEW_PICTURE","DETAIL_PAGE_URL");//IBLOCK_ID и ID обязательно должны быть указаны, см. описание arSelectFields выше
	$arFilter = Array("IBLOCK_ID" => 30,"ID" => $arResult['PROPERTIES']['BONUS_PRODUCT']['VALUE']);
	$res = CIBlockElement::GetList(Array(), $arFilter, false, Array("nPageSize"=>50), $arSelect);
	while($ob = $res->GetNextElement()){ 
		$arResult['BONUS_PRODUCTS'][$ob->GetFields()['ID']] = $ob->GetFields();  
