...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["register_submit_button"] <> '' && !$USER->IsAuthorized())
{
	...
	else // if there's no any errors - create user
	{
		...
		if (intval($ID) > 0)
		{
			...
			$event = new CEvent;
			$event->Send("NEW_USER", SITE_ID, $arEventFields,"N", 203);

			CModule::IncludeModule('iblock'); 
			global $USER;
			$el = new CIBlockElement;
			$PROP = array();
			$PROP[1510] = $_REQUEST["REGISTER"]["EVENT"];
			$PROP[1509] = $arResult['VALUES']["USER_ID"];
			$PROP[1511] = $_GET["code"];
			$PROP[1512] = date("d.m.Y", strtotime('+1 month'));
			$PROP[1514] = date("d.m.Y");
			$arLoadProductArray = Array(
			  "MODIFIED_BY"    => 1,
			  "IBLOCK_SECTION_ID" => 9083,
			  "IBLOCK_ID"      => 100,
			...
			if($PRODUCT_ID = $el->Add($arLoadProductArray)){
				...
				if($arParams["REGISTRATION_STATUS"] == "first"){
					CIBlockElement::SetPropertyValuesEx($arParams["LINK_ID"], false, array(1514 => date("d.m.Y")));
				}else{
					CIBlockElement::SetPropertyValuesEx($arParams["LINK_ID"], false, array(1516 => date("d.m.Y")));
				}
			}
			else{
