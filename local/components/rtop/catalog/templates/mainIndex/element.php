...
$arFilter = Array("IBLOCK_ID"=>$arParams['IBLOCK_ID'], "CODE"=>$GLOBALS['PAGE'][count($GLOBALS['PAGE'])-2], "ACTIVE"=>"Y");
$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
while($ob = $res->Fetch()){
	$arCur = $ob;
}
if($arCur){
	LocalRedirect('/products/'.$arCur['CODE'].'/',false,"301 Moved permanently");
}else{
	$arSelect = Array("ID",'CODE');
	$arFilter = Array("IBLOCK_ID"=>$arParams['IBLOCK_ID'], "PROPERTY_OLDCODE"=>$GLOBALS['PAGE'][count($GLOBALS['PAGE'])-2], "ACTIVE"=>"Y");
	$res = CIBlockElement::GetList(Array(), $arFilter, false, Array(), $arSelect);
	while($ob = $res->Fetch()){
		$arCur = $ob;
	}
