...
if($arParams["SEF_MODE"] == "Y")
{
	...
	if($b404 && CModule::IncludeModule('iblock'))
	{
		...
		{	
			if(!empty($_REQUEST['URL']) && !preg_match('/(\?)|(\&)|(\/)/', $_REQUEST['URL']))
			{
				$componentPage = "search";				
				...
				$result = $entity_data_class::getList(['filter' => ['=UF_URL' => $_REQUEST['URL']], 'select' => ['UF_REQUEST']])->fetch();
				$_REQUEST["q"] = (!empty($result) ? $result['UF_REQUEST'] : str_replace('_', ' ', $_REQUEST['URL']));
			}else{
