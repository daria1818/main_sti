...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	public $filePath = "/home/bitrix/www/ses_log_jeiorjioashdf/sss.log";
	...
	*/
	...
	function makeFilter($FILTER_NAME)
	{
		...
		if($file = fopen("/home/bitrix/www/ses_log_jeiorjioashdf/sss1.log", 'w')){
	      $data = print_r($gFilter, true);
	      fwrite($file,  $data);
	      fclose($file);
	    }
		...
		if($file = fopen("/home/bitrix/www/ses_log_jeiorjioashdf/sss2.log", 'w')){
			      $data = print_r(array_merge($gFilter, $arFilter), true);
			      fwrite($file, $data);
			      fclose($file);
			    }
		if($file = fopen("/home/bitrix/www/ses_log_jeiorjioashdf/sss3.log", 'w')){
	...
	public function searchValue($item, $lookupValue)
	{
		...
		if (!$error)
		{
			$encodedValue = rawurldecode($searchValue);
			foreach($item as $itemId => $arValue)
			{
				if (strtolower($encodedValue) === strtolower($arValue["VALUE"])){
					return $itemId;
				}
			}
		}
