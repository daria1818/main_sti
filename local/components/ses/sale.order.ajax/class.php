...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	public static $checkerStart = true;
    ...
    public static function timing($checkpoint_name)
	{

		if(self::$checkerStart == true)
		{
			self::$start = self::getMicroTime();
			self::$checkerStart = false;
		}
            


		$diff = microtime(true) - self::$start;
            //self::$lastTime = $diff - self::$lastTime;


		$str = "Время выполнения $checkpoint_name: " . sprintf('%.6f sec.', $diff);
            //$str1 = "Разница: " . self::$lastTime;

		Logger::writeLog([self::$start, $str, $str1], '/logs/', 'timings');
		return true;
	}

	public function onPrepareComponentParams($arParams)
	{
		self::timing(__FUNCTION__);
		global $APPLICATION;
	...
	protected function saveOrder($saveToSession = false)
	{
		...
		self::timing('saveOrder_before');
		$res = $this->order->save();
		self::timing('saveOrder_after');
