...
*/
class CBitrixCatalogSmartFilter1 extends CBitrixComponent
{
	public $IBLOCK_ID = 0;
		...
		if ($apply)
		{
			...
			foreach($this->arResult["ITEMS"] as $PID => $arItem)
			{
				...
				else
				{
					...
					{
						if (
							($ar["CHECKED"] || $ar["CONTROL_ID"] === $checkedControlId)
							&& mb_strlen($ar["VALUE"]) // Изменено с URL_ID на VALUE
						)
						{
							$smartPart[] = $ar["VALUE"]; // Изменено с URL_ID на VALUE
						}
					}
				}
				...
				if ($smartPart)
				{
					if ($arItem["CODE"])  // Изменено с CODE на NAME
						array_unshift($smartPart, toLower($arItem["CODE"])); // Изменено с CODE на NAME
					else
