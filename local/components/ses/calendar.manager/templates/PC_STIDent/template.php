	...
	<div class="container">
				...
				<?/*<div class="page-header">
					<div class="page-header__info">
						...
						<p class="page-header__account-name">
							<?=$arResult["USER_INFO"]["NAME"] . " ({$arResult["USER_INFO"]["ROLE"]})"?>
				</p>
			</div>
		...
		<div class="page-filters page-filters--center">
			<form action="" method="_POST" name="city-form" id="city-form">
				...
				<? if ($arResult["USER_INFO"]['ROLE'] == 'Администратор STIDent'){?>
				<select class="select-lektor" name="lektor" data-title="Выберите Лектора">
					...
					<? if(false) continue; //$user['ID'] == 3 $user['ID'] == 2 || $user['ID'] == 1?>
				...
				<select class="select-spec" name="spec" data-title="Выберите Специализацию">
					<option value="">Все специализации</option>
					<? foreach ($arResult['descFieldList']["UF_SPEC"] as $key => $value) {?>
			...
			<div class="page-option__month">
				<a class="page-option__btn-prev month-prev">
					<svg class="arrow-icon">
						<use href="/gbt/sda/assets/svg/sprite.svg#arrow-icon"></use>
					</svg>
				</a>
...
<div class="modal modal-course">
	<form class="modal-course__content" action="" novalidate>
		...
		<div class="modal-course__inputs">
			...
			<div class="modal-course__theme">
				<input type="text" name="UF_THEME" placeholder="Введите тему курса" value="">
			</div>

			<?if ($arResult["USER_INFO"]['ROLE'] == "Администратор STIDent"){?>
			<div class="modal-course__select modal-course__lector">
			...
			<div class="modal-course__select">
				<select name="UF_TYPE" id="" required data-title="Выберите тип курса">
					<? foreach ($arParams["FILTER"]["UF_TYPE"] as $value) {?>
					<option value="<?=$value?>"><?=$arResult["descFieldList"]["UF_TYPE"][$value]?></option>
					<?}?>
				</select>
			...
			<div class="modal-course__select">
				<select name="UF_SPEC" id="" required data-title="Выберите Специализацию">
					<? foreach ($arResult['descFieldList']["UF_SPEC"] as $key => $value) {?>
					<option value="<?=$key?>"><?=$value?></option>
					<?}?>
				</select>
			...
			<div class="modal-course__two-row">
				<div class="modal-course__datetime">
					<input type="datetime-local" name="UF_DATE" value="" required>
				</div>
				<div class="modal-course__datetime-end">
					<input type="datetime-local" name="UF_DATE_END" value="" required>
				</div>
			</div>
			<div class="modal-course__price">
				<input type="number" name="UF_PRICE" placeholder="Стоимость" required />
			</div>
			...
			<input type="hidden" name="UF_CITY" value="365">
			<input type="hidden" name="UF_LOCATION" value="УЦ STIDent">
			<input type="hidden" name="UF_THIS_LOC_DENT" value="1">
