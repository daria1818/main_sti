...
class SaleProductsGiftBasketComponent extends ElementList
{
	...
	protected function customGetGiftIds()//самопал
	{
		$collections = [];
		$collections = $this->giftManager->customCreateGiftIds(
			$this->arParams['FULL_DISCOUNT_LIST']
		);
		$this->arParams['COL_CUST'] = $collections;
		return $collections;
	}
	...
	protected function getDeferredProductIds()
	{
		...
		$filePath = $_SERVER["DOCUMENT_ROOT"]."/ses_log_jeiorjioashdf/salesARR.log";
			 if($file = fopen($filePath, 'w')){
	           $data = print_r($productIds, true);
	           fwrite($file, $data);
	           fclose($file);
	        }
		return $productIds;
