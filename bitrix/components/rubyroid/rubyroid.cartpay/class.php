...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function initialLoadAction()
	{
		...
		if($this->initializationRB())
		{

			//$result = RB::sendRqs("get_user_balance", array('email' => $USER->GetEmail()));
			// $result = array(
			// 	"id" => $USER->GetID(),
			// 	"ballance" => $_SESSION['RB_BALANCE'],
			// ); 

			$result = array(
				"id" => $USER->GetID(),
				"ballance" => $this->arResult['TOTAL_RENDER_DATA']['RB_MAX_PAY'],
			); 
			...
			//$result["ballance"] =  $result["ballance"] * $exchange_rate;
			//echo $result["ballance"];
			if(!!$result['id'] && isset($result['ballance']))
			{

				$this->arResult['RB'] = $result;
				if($this->arResult['RB']['ballance'] > 0)
				{
					$_SESSION['RB_BALANCE'] = $this->arResult['RB']['ballance'];
					$this->arResult['RB']['balance_formated'] = trim($this->arResult['RB']['ballance'] . " " . RB::NumberWordEndings($this->arResult['RB']['ballance']));
					if(!empty($this->arResult['rb_pay_count']))
						$sum = $this->arResult['rb_pay_count']+$this->arResult['allSum'];
					else
						$sum = $this->arResult['allSum'];
					$max = RB::getMaxPayBasketPage($result['ballance'], $sum, $result['ballance']);
					if(!empty($max))
					{
						$this->arResult['RB'] = array_merge($this->arResult['RB'], $max);
					}
				}
				else
				{
					RB::unsetSessionParams();
				}
			}
			else
			{
				RB::unsetSessionParams(); // если это закоментировать то бонусы работают
			}
		}
		$this->IncludeComponentTemplate();
	}

	protected function initializationRB()
	{
		global $USER;
		...
		if(Loader::includeModule("rubyroid.bonusloyalty"))
		{
			...
			{
				$this->arResult['RB']['AUTRZ_TEXT'] = Loc::getMessage('AUTRZ_TEXT');
			}
		}
	...
	protected function recalculateAction()
	{
		...
		// {
			$result['BASKET_DATA']['GIFTS_RELOAD'] = true;
				?><script>console.log('AjaxReload')</script><?
		// }
	...
	protected function getBasketTotal()
	{
		...
		if($this->initializationRB())
		{
			$_SESSION['RB_INPUT'] = $this->request->get('rb_input') ? $this->request->get('rb_input') : $_SESSION['RB_INPUT'];

			$_SESSION['RB_PAY_COUNT'] = $this->request->get('pay_count') >= 0 && $this->request->get('pay_count') != null ? $this->request->get('pay_count') : $_SESSION['RB_PAY_COUNT'];
			$_SESSION['RB_MAX_PAY_COUNT'] = $this->request->get('rb_max_pay') >= 0 && $this->request->get('rb_max_pay') != null ? $this->request->get('rb_max_pay') : $_SESSION['RB_PAY_COUNT'];
			
			if($_SESSION['RB_PAY_COUNT'] < $_SESSION['RB_MAX_PAY_COUNT']){
				$input = $_SESSION['RB_PAY_COUNT'] && $_SESSION['RB_INPUT'] == 'on' ? $_SESSION['RB_PAY_COUNT'] : $_SESSION['RB_BALANCE'];
			}else{
				$_SESSION['RB_PAY_COUNT'] = $_SESSION['RB_MAX_PAY_COUNT'];
				$input = $_SESSION['RB_PAY_COUNT'] && $_SESSION['RB_INPUT'] == 'on' ? $_SESSION['RB_PAY_COUNT'] : $_SESSION['RB_BALANCE'];
			}
			//$ballance = $_SESSION['RB_BALANCE'] ? $_SESSION['RB_BALANCE'] : false;
			$ballance = $_SESSION['RB_MAX_PAY_COUNT'] ? $_SESSION['RB_MAX_PAY_COUNT'] : false;
			
			//control
			if($input != null)
				$max = RB::getMaxPayBasketPage($input, $result['allSum'], $ballance);

			if(!empty($max))
			{
				$result['rb_max'] = $max['RB_MAX_PAY'];
			}

			if($_SESSION['RB_INPUT'] == 'on' && $_SESSION['RB_PAY_COUNT'] > 0)
			{				
				if(!empty($result['rb_max']))
				{					
					if($result['rb_max'] < $_SESSION['RB_PAY_COUNT'])
					{
						$_SESSION['RB_PAY_COUNT'] = $result['rb_max'];
						$result['rb_max_display'] = $max['RB_MAX_PAY_DISPLAY'];
					}
				}
				// 
				$result['allSum'] = $result['allSum']-$_SESSION['RB_PAY_COUNT'];
				$result['allSum_FORMATED'] = CCurrencyLang::CurrencyFormat($result['allSum'], $result['CURRENCY'], true);
				$result['rb_pay_count'] = $_SESSION['RB_PAY_COUNT'];
			}

			if($_SESSION['RB_INPUT'] == 'off')
			{
