...
final class Manager
{
	...
	public function getCollectionsByBasket(Basket $basket, array $discounts = null, array $appliedDiscounts = null)
	{
		...
		foreach($potentialGiftData as $giftData)
		{
			...
			{
				$giftCollection[] = new Gift($value); //id подарка
			}
			unset($value);
	...
	public function customGetGiftIds(array $discounts){ //самопал
        ...
        //    $data = print_r($sortDiscount, true);
        //    fwrite($file, $data);
        //    fwrite($file, "\n----------------------------------------------\n");
        // }
        // fclose($file);
	...
	private function customSortDiscount($method = "top", array $discounts){
		foreach ($discounts as $key => $discount) {
			$strCONDITIONS = stripos(serialize($discount["CONDITIONS"]),"CondBsktFldProduct");
			$strACTION = stripos(serialize($discount["ACTION"]),"CondBsktFldProduct");

			if ($strCONDITIONS !== false || $strACTION !== false) {
				$sortDiscount["1TOP"][$key] = $discount;
			}else{
				$sortDiscount["2BOT"][$key] = $discount;
			}
		}
		$method == "top" ? ksort($sortDiscount) : krsort($sortDiscount);
		return $sortDiscount;
